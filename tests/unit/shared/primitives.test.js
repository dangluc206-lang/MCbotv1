...
test('Redactor distinguishes shared references from cycles and bounds self-caused errors',()=>{const shared={safe:7};const sanitized=Redactor.sanitize({left:shared,right:shared});assert.equal(sanitized.left.safe,7);assert.equal(sanitized.right.safe,7);const error=new Error('self');error.cause=error;assert.equal(Redactor.sanitize(error).cause,'[Circular]');});
