...
class CraftingResultVerifier {
    ...
    before(outputId, inputIds = [], { inventorySource = 'all' } = {}) {
        ...
        for (const inputId of inputIds || []) {
            if (!inputId) continue;
            inputCounts[inputId] = this.#countViews(views, inputId);
        }
        return {
    ...
    async after(outputId, before, {
        ...
        let last = null;
        const expectedIdentity = this.guiKnowledge?.getStrongIdentity?.(outputId) || null;
        const syncEvidence = this.inventorySync
            ? await this.inventorySync.waitForStable({
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
            const views = this.#readViews(inventorySource);
            let beforeCounted = this.#countViews(before.views || [before.snapshot], outputId);
            let afterCounted = this.#countViews(views, outputId);
            let learnedIdentity = null;
            const snapshotMmoCandidates = this.#positiveMmoCandidatesAcrossAllViews(
                before.views || [before.snapshot],
                views
            );

            if (afterCounted.count <= beforeCounted.count && this.guiKnowledge?.learnLogicalItem) {
                // Compare normalized inventory as one logical player inventory,
                // not source-by-source. currentWindow and bot.inventory are two
                // mirrors of the same slots and either one can be stale or can
                // appear/disappear between the before/after captures.
                let candidate = this.#bestPositiveMmoDeltaAcrossAllViews(
                    before.views || [before.snapshot],
                    views,
                    expectedDelta
                );
                ...
                if (!candidate) {
                    candidate = this.#bestPositiveMmoDeltaAcrossViews(
                        before.views || [before.snapshot],
                        views,
                        expectedDelta
                    );
                ...
                // B1 -> B2 crafting consumes the vanilla input directly from /kho,
                // so inventory snapshots can remain at output=0 even though the
                // server has already inserted a custom MMOItems output. In that
                // case learn the output identity from the actual slot-change
                // events captured after the quantity click.
                if (!candidate) {
                ...
                if (candidate) {
                    ...
                    beforeCounted = this.#countViews(before.views || [before.snapshot], outputId);
                    afterCounted = this.#countViews(views, outputId);
            ...
            const eventEvidence = this.#eventEvidence(
                before?.verificationStartedAt || before?.capturedAt || 0,
                outputId,
                inputRequirements,
                inventorySource
            ...
            const inputEvidence = this.#inputConsumptionEvidence(before, views, inputRequirements, eventEvidence);
            const inputVerified = inputEvidence.some(entry => entry.verified);

            let verificationMode = 'none';
            if (snapshotOutputDelta >= expectedOutput) verificationMode = 'output-snapshot-delta';
            else if (Number(eventEvidence.outputDelta || 0) >= expectedOutput) verificationMode = 'output-event-delta';
            else if (inputEvidence.some(entry => entry.snapshotVerified)) verificationMode = 'input-snapshot-consumption';
            else if (inputEvidence.some(entry => entry.eventVerified)) verificationMode = 'input-event-consumption';

            last = {
                verified: outputVerified || inputVerified,
                verificationMode,
                ...
                snapshotMmoCandidates: snapshotMmoCandidates.map(candidate => ({
                    ...
                    bySourceBefore: candidate.bySourceBefore,
                    bySourceAfter: candidate.bySourceAfter
                })),
                attempt,
                learnedIdentity,
                inventorySource,
                syncEvidence: syncEvidence ? {
                    stable: Boolean(syncEvidence.stable),
                    timedOut: Boolean(syncEvidence.timedOut),
                    elapsedMs: syncEvidence.elapsedMs ?? null,
                    eventCount: syncEvidence.eventCount ?? 0,
                    stablePasses: syncEvidence.stablePasses ?? 0,
                    quietForMs: syncEvidence.quietForMs ?? 0,
                    expectedIdentity: syncEvidence.expectedIdentity || null,
                    beforeIdentityCount: syncEvidence.beforeIdentityCount ?? null,
                    afterIdentityCount: syncEvidence.afterIdentityCount ?? null,
                    identityDelta: syncEvidence.identityDelta ?? null,
        ...
        return last || {
            ...
            views: before.views || [before.snapshot],
            countsBySource: before.countsBySource || {},
            beforeCountsBySource: before.countsBySource || {},
            inputEvidence: [],
            eventEvidence: { outputDelta: 0, outputBySource: {}, inputs: {}, eventCount: 0, mmoCandidates: [] },
