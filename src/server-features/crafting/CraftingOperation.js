...
class CraftingOperation {
    ...
    async execute(recipeId, amount) {
        ...
        try {
            before = this.resultVerifier.before(recipe.output, Object.keys(recipe.inputs || {}), { inventorySource: 'bot-inventory' });
            ...
            let session = await this.#openMineralsRoot(rootSource);
            this.#trace('CRAFT /ks READY', stage, { recipeId, resource: recipe.output, phase: 'OK', title: session?.window?.title || null });
            ...
            this.#trace('CRAFT MENU READY', stage, { recipeId, resource: recipe.output, phase: 'OK', title: session?.window?.title || null });
            ...
            this.#trace('CRAFT QUANTITY MENU READY', stage, { recipeId, resource: recipe.output, phase: 'OK', title: quantitySession?.window?.title || null });
            ...
            await this.guiManager.click(quantitySlot);
            ...
            if (this.config.resultDelayMs > 0) await Timeout.delay(this.config.resultDelayMs);
            ...
            if (!verification.verified) {
                throw this.#flow('CRAFTING_OUTPUT_NOT_VERIFIED', stage, `verify quantity ${quantity}`, recipe.output, {
                    ...baseDetails,
    ...
    async #openMineralsRoot(rootSource) {
        ...
        if (current?.active && current?.source?.command === '/ks'
            ...
            if (this.config.openSettleMs > 0) await Timeout.delay(this.config.openSettleMs);
            return current;
        }
        ...
        for (let attempt = 1; attempt <= attempts; attempt += 1) {
            try {
                ...
                if (this.guiManager.current()?.active) {
                    ...
                    if (this.config.commandCloseSettleMs > 0) await Timeout.delay(this.config.commandCloseSettleMs);
                }
                ...
                if (retryMs > 0) await Timeout.delay(retryMs);
            }
        }
    ...
    #flow(code, step, action, resource, details) {
        return new FlowError(this.#messageForCode(code, resource), {
            code, subsystem: 'crafting', operation: 'CraftingOperation', step, action, resource, details
        });
    }
    ...
    #fallbackRecipeSlot(window, recipe) {
        ...
        return slots.findIndex(item => item && this.itemResolver.matches(item, recipe.menuItemId, 'crafting-menu').matched);
    }

    #fallbackEntrySlot(window) {
        const slots = window?.slots || [];
        if (Number.isInteger(this.config.entrySlot) && this.config.entrySlot >= 0 && slots[this.config.entrySlot]) return this.config.entrySlot;
        return slots.findIndex(item => item && this.itemResolver.matches(item, this.config.entryMenuItemId, 'minerals-menu').matched);
    }
