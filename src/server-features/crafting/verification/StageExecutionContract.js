...
class StageExecutionContract {
    ...
    requireInputReady({ stage, logicalId, available, required, context = null }) {
        ...
        this.logger?.debug?.('CRAFT STAGE INPUT READY', { stage, logicalId, available: actual, required: need });
    ...
    verifyOutput({ stage, logicalId, before, after, expectedDelta, context = null }) {
        ...
        this.logger?.debug?.('CRAFT STAGE OUTPUT VERIFIED', { stage, logicalId, before: b, after: a, expectedDelta: expected, actualDelta: delta });
    ...
    requireSettled({ stage, logicalId, settlement, context = null }) {
        ...
        this.logger?.debug?.('CRAFT STAGE SETTLED', { stage, logicalId, count: settlement.count, elapsedMs: settlement.elapsedMs });
        return settlement;
    }
    ...
    handoff({ from, to, generation = null, context = null }) {
        ...
        this.logger?.info?.('CRAFT STAGE HANDOFF READY', { from, to, generation });
