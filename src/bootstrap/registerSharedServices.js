...
const SharedResourceLeaseRegistry = require("../core/resources/SharedResourceLeaseRegistry");
...
function registerSharedServices({
  ...
  if (!loggerOutput) {
    runtimeLogOutput = new RuntimeLogOutput({
  ...
  const serverProfiles = createServerProfileRegistry(
    configuration.registry.require("server"),
    {
      commands: configuration.registry.require("commands"),
      commandResponses: configuration.registry.require("commandResponses"),
      skyCommands: configuration.registry.require("skyCommands"),
      authentication: configuration.registry.require("serverLogin"),
      join: configuration.registry.require("skyblock"),
      guiWindows: configuration.registry.require("guiWindows"),
      guiIdentity: configuration.registry.require("guiIdentity"),
  ...
  const multiBot = app?.multiBot || {};
  const connectionAttempts = new ConnectionAttemptCoordinator({
    minSpacingMs: Number(multiBot.connectionStartSpacingMs ?? 10000),
    postSuccessSpacingMs: Number(
      multiBot.postSuccessSpacingMs ??
        multiBot.connectionStartSpacingMs ??
        10000,
    ),
    transientFailureCooldownMs: Number(
      multiBot.transientFailureCooldownMs ?? 15000,
    ),
    connectionResetCooldownMs: Number(
      multiBot.connectionResetCooldownMs ?? 20000,
    ),
    lostConnectionCooldownMs: Number(
      multiBot.lostConnectionCooldownMs ?? 20000,
    ),
    loginTooFastCooldownMs: Number(multiBot.loginTooFastCooldownMs ?? 30000),
    logger: loggerFactory.create("ConnectionAttempts"),
  });
  const itemRegistry = new ItemRegistry(
    configuration.registry.require("items"),
  );
  const itemNormalizer = new ItemNormalizer();
  const composite = new CompositeItemMatcher({
    name: new NameMatcher(),
    lore: new LoreMatcher(),
    nbt: new NbtMatcher(),
    material: new MaterialMatcher(),
    identity: new IdentityMatcher(),
  });
  ...
  const configMutations = new KeyedMutationCoordinator({
    name: "ConfigMutationCoordinator",
    logger: loggerFactory.create("ConfigMutations"),
  });
  const sharedResourceLeases = new SharedResourceLeaseRegistry({
    logger: loggerFactory.create("SharedResourceLeases"),
  });
  return {
    loggerFactory,
    eventBus,
