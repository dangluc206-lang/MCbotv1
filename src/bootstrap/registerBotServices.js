...
function registerBotServices({profile,configuration,shared}){
 const botId=profile.id;const logger=shared.loggerFactory.create(`BotRuntime:${botId}`);const appConfig=configuration.registry.require('app');const runtimeFailureConfig=appConfig.diagnostics.runtimeFailures;const failurePolicy=appConfig.diagnostics.circuitBreaker;const selectedServerProfileId=profile.serverProfile||configuration.registry.require('server').defaultProfile||'default';const serverProfile=shared.serverProfiles.require(selectedServerProfileId);const server=serverProfile.endpoint;const eventBus=new EventBus();const identity=new BotIdentity({botId,displayName:profile.displayName,username:profile.username,role:profile.role,serverProfile:serverProfile.id,serverProfileRevision:serverProfile.revision});const context=new BotContext(botId);const state=new BotState();const connectionStateBinding=createConnectionStateBinding({botId,state,eventBus,context});
 const operationConfig=appConfig.operations||{};const queue=new OperationQueue({maxPending:operationConfig.maxPending??128});const lockPolicy=new OperationLockPolicy();const timeoutPolicy=new OperationTimeoutPolicy();const operationManager=new OperationManager({botId,queue,lockPolicy,timeoutPolicy,logger,config:operationConfig});
 const commands=new CommandRegistry(serverProfile.requireCatalog('commands'));const commandResolver=new CommandResolver({registry:commands});const commandGuard=new CommandGuard({context,minimumIntervalMs:appConfig.commandIntervalMs||250});const commandExecutor=new CommandExecutor({context,guard:commandGuard});const responseMatcher=new ResponseMatcher();const confirmation=new CommandConfirmation({eventBus,matcher:responseMatcher,context});const commandService=new CommandService({botId,resolver:commandResolver,executor:commandExecutor,confirmation,responseRules:serverProfile.requireCatalog('commandResponses')});const slashCommandService=new SlashCommandService({executor:commandExecutor});
 const itemRegistry=new ItemRegistry(serverProfile.requireCatalog('items'));const itemResolver=new ItemResolver({registry:itemRegistry,matcher:shared.itemResolver.matcher});
 const guiState=new GuiState();const guiRegistry=new GuiRegistry(serverProfile.requireCatalog('guiWindows'));const titleMatcher=new TitleMatcher();const layoutMatcher=new LayoutMatcher();const fingerprintMatcher=new SlotFingerprintMatcher({itemResolver:itemResolver});const windowMatcher=new WindowMatcher({titleMatcher,layoutMatcher,fingerprintMatcher});const guiIdentityEngine=new GuiIdentityEngine({registry:guiRegistry,titleMatcher,layoutMatcher,fingerprintMatcher,config:serverProfile.requireCatalog('guiIdentity')});const detector=new GuiDetector({registry:guiRegistry,windowMatcher,identityEngine:guiIdentityEngine});const slotRegistry=new SlotRegistry(serverProfile.requireCatalog('guiSlots'));const slotResolver=new SlotResolver({slotRegistry,itemResolver:itemResolver});const slotValidator=new SlotValidator();const slotInspector=new SlotInspector({normalizer:shared.itemNormalizer});const clickQueue=new ClickQueue();const clickGuard=new ClickGuard({context,slotValidator});const clickExecutor=new ClickExecutor({context});const clickVerifier=new ClickVerifier({eventBus,context});const guiManager=new GuiManager({botId,context,state:guiState,detector,clickQueue,clickGuard,clickExecutor,clickVerifier,eventBus,logger});
 const observationConfig=configuration.registry.require('guiObservation');const guiNormalizer=new GuiStructureNormalizer({itemNormalizer:shared.itemNormalizer});const guiObservationStore=new GuiObservationStore({baseDir:path.resolve(configuration.loader.baseDir,observationConfig.directory),botId,logger});const recipeDefinitionsForLearning=serverProfile.requireCatalog('recipes');const guiBootstrapMappings=[{recordKeys:['ks__menu_crafting','ks__slot-16'],entries:Object.entries(recipeDefinitionsForLearning).map(([id,recipe])=>({roleId:`recipe:${id}`,logicalItemId:recipe.menuItemId,bootstrapSlot:recipe.menuSlot}))}];const guiKnowledge=observationConfig.enabled===false?null:new GuiKnowledgeRegistry({botId,normalizer:guiNormalizer,store:guiObservationStore,itemResolver:itemResolver,bootstrapMappings:guiBootstrapMappings,logger});const guiObservationService=observationConfig.enabled===false?null:new GuiObservationService({botId,eventBus,guiManager,knowledgeRegistry:guiKnowledge,debounceMs:observationConfig.debounceMs,logger});
 ...
 const inventoryReader=new InventoryReader({botId,context,normalizer:shared.itemNormalizer,logger});const inventoryScanner=new InventoryScanner({resolver:itemResolver,guiKnowledge});const inventoryCounter=new InventoryCounter({scanner:inventoryScanner});const inventoryObservationConfig=configuration.registry.require('inventoryObservation');const inventoryObservationStore=new InventoryObservationStore({baseDir:path.resolve(configuration.loader.baseDir,inventoryObservationConfig.directory||'data/runtime/inventory'),botId,logger});const inventoryObservationService=inventoryObservationConfig.enabled===false?null:new InventoryObservationService({botId,context,eventBus,reader:inventoryReader,store:inventoryObservationStore,normalizer:shared.itemNormalizer,debounceMs:inventoryObservationConfig.debounceMs,historyLimit:inventoryObservationConfig.historyLimit||300,logger});const inventorySyncService=new InventorySyncService({botId,context,reader:inventoryReader,observation:inventoryObservationService,logger,config:inventoryObservationConfig.postActionSync||{}});
 const storageConfig=serverProfile.requireCatalog('storage');const capacityReader=new KhoCapacityReader({itemResolver:itemResolver,config:storageConfig});const khoReader=new KhoReader({itemResolver:itemResolver,capacityReader,config:storageConfig});const sellGuiReader=new SellGuiReader({itemResolver:itemResolver,config:storageConfig});const khoSellOperation=new KhoSellOperation({commandService,guiManager,context,reader:sellGuiReader,config:storageConfig,logger});const storage=new KhoService({commandService,guiManager,reader:khoReader,sellOperation:khoSellOperation,config:storageConfig,guiKnowledge,operationManager,context,logger});const khoWithdrawOperation=new KhoWithdrawOperation({storage,guiManager,context,itemResolver:itemResolver,inventoryReader,inventoryCounter,config:storageConfig,logger});storage.setWithdrawOperation(khoWithdrawOperation);
 const pvConfig=serverProfile.requireCatalog('personalVault');const pvReader=new PersonalVaultReader({itemResolver:itemResolver,guiKnowledge,normalizer:shared.itemNormalizer,storageSlots:pvConfig.storageSlots});const pvTransfer=new PersonalVaultTransfer({guiManager,itemResolver:itemResolver,guiKnowledge,storageSlots:pvConfig.storageSlots,logger});const personalVault=new PersonalVaultService({commandService,guiManager,reader:pvReader,transfer:pvTransfer,config:pvConfig,guiKnowledge,inventoryReader,inventoryCounter,operationManager,context,logger});
 const mineralConfig=serverProfile.requireCatalog('minerals');const mineralConversionConfig=serverProfile.requireCatalog('mineralConversions');const mineralOperation=new MineralConversionOperation({commandService,guiManager,context,itemResolver:itemResolver,guiKnowledge,config:mineralConfig,conversionConfig:mineralConversionConfig,logger});const minerals=new MineralService({operation:mineralOperation,operationManager,context});
 const smeltingConfig=serverProfile.requireCatalog('smelting');const smeltingOperation=new SmeltingOperation({commandService,guiManager,context,itemResolver:itemResolver,guiKnowledge,config:smeltingConfig,logger});const smelting=new SmeltingService({operation:smeltingOperation,operationManager,context});const b1Materials=new B1StorageMaterialService({storage,minerals,smelting,conversionConfig:mineralConversionConfig,smeltingConfig,recipeConfig:recipeDefinitionsForLearning,targetId:configuration.registry.require('b5').targetId,serverProfile,logger});
 const recipeRegistry=new CraftingRecipeRegistry(serverProfile.requireCatalog('recipes'));const quantityResolver=new CraftingQuantityResolver(mineralConfig.crafting||{});const resultVerifier=new CraftingResultVerifier({inventoryReader,inventoryCounter,guiKnowledge,inventoryObservation:inventoryObservationService,inventorySync:inventorySyncService});const craftingOperation=new CraftingOperation({commandService,guiManager,context,itemResolver:itemResolver,recipeRegistry,quantityResolver,resultVerifier,guiKnowledge,config:mineralConfig.crafting||{},logger});const crafting=new CraftingService({operation:craftingOperation,operationManager,context});const materialCalculator=new MaterialCalculator({recipeRegistry});const craftingPlanner=new CraftingPlanner({recipeRegistry,materialCalculator});const craftingTiers=serverProfile.requireCatalog('craftingTiers');const b5Config=configuration.registry.require('b5');const b5Planner=new B5Planner({planner:craftingPlanner,targetId:b5Config.targetId,tiers:craftingTiers});const b5ExecutionPlanner=new B5ExecutionPlanner();const b5TraceRecorder=new B5TraceRecorder({botId,serverProfile,historyLimit:100,logger});const b5Planning=new B5PlanningService({storage,personalVault,inventoryReader,inventoryCounter,b5Planner,executionPlanner:b5ExecutionPlanner,materialCalculator,recipeRegistry,tiers:craftingTiers,b1Materials,config:b5Config,guiDataMaxAgeMs:Number(observationConfig.semanticCacheMs||5000)});const b5Automation=new B5AutomationService({planningService:b5Planning,crafting,personalVault,storage,b1Materials,inventoryReader,inventoryCounter,recipeRegistry,operationManager,context,traceRecorder:b5TraceRecorder,config:b5Config,logger});
 const connectionStateView=new ConnectionStateView({context});const islandConfig=configuration.registry.require('island');const islandOperation=new IslandTeleportOperation({commandService,positionService,eventBus,connectionState:connectionStateView,botId,config:islandConfig});const island=new IslandService({operation:islandOperation,operationManager,context});
 const dungeonConfig=configuration.registry.require('dungeon');const destinations=new DungeonDestinationRegistry(dungeonConfig.destinations||{});const dungeonOperation=new DungeonTeleportOperation({botId,context,commandService,guiManager,itemResolver:itemResolver,guiKnowledge,destinations,eventBus,config:dungeonConfig});const dungeon=new DungeonService({operation:dungeonOperation,operationManager,context});
 const skyblockConfig=serverProfile.requireBinding('join');const skyTarget=profile.skyblockSelection||skyblockConfig.defaultSelection;const skyblockOperation=new SkyblockJoinOperation({botId,context,commandService,guiManager,guiKnowledge,eventBus,config:skyblockConfig});const skyblock=new SkyblockService({operation:skyblockOperation,operationManager,context});const skyblockAutoJoin=new SkyblockAutoJoinService({botId,context,eventBus,skyblock,config:{...(skyblockConfig.modeJoin||{}),selection:skyTarget},dailyRecovery:configuration.registry.require('dailyRecovery'),logger});const skyCommandService=new SkyCommandService({botId,context,slashCommandService,skyblockReadiness:skyblockAutoJoin,config:serverProfile.requireCatalog('skyCommands'),logger});
 const sessionManager=new SessionManager({botId});const connectionFactory=new ConnectionFactory({botFactory:shared.botFactory});const resourcePackAutoAccept=new ResourcePackAutoAcceptService({botId,context,eventBus,config:configuration.registry.require('resourcePack'),logger});const serverLoginService=new ServerLoginService({botId,context,eventBus,commandService,password:profile.password,config:serverProfile.requireBinding('authentication'),logger});const connectionManager=new ConnectionManager({botId,context,sessionManager,connectionFactory,profile,server,eventBus,logger,attemptCoordinator:shared.connectionAttempts,readyTimeoutMs:profile.readyTimeoutMs||30000,autoConnect:profile.runtimeAutoConnect??profile.enabled});const reconnectManager=new ReconnectManager({botId,connectionManager,context,eventBus,policy:profile.reconnect||{},dailyRecovery:configuration.registry.require('dailyRecovery'),attemptCoordinator:shared.connectionAttempts,logger});const connectionEventBinding=createConnectionEventBinding({botId,context,eventBus});
 const fishingConfig=resolveFishingConfig(configuration.registry.require('fishingMode'),profile.fishing||{});const afkOccupancyParser=new AfkAreaOccupancyParser({itemNormalizer:shared.itemNormalizer,config:fishingConfig});const afkAreas=new AfkAreaService({botId,context,commandService,guiManager,eventBus,positionService,occupancyParser:afkOccupancyParser,operationManager,config:fishingConfig,logger});const fishing=new FishingService({context,rotationService,config:fishingConfig,logger});const fishingPacketObserver=new ConnectionPacketObserver({botId,context,eventBus,config:fishingConfig,logger});const fishingPositionGuard=new FishingPositionGuard({positionService,connectionState:connectionStateView,config:fishingConfig});const fishingWorldReadiness=new FishingWorldReadinessService({context,connectionState:connectionStateView,config:fishingConfig,logger});const fishingMovement=new FishingMovementOperation({botId,context,connectionState:connectionStateView,operationManager,controlStateManager,rotationService,positionService,config:fishingConfig,logger});const fishingMovementProbe=new FishingMovementProbeService({movementOperation:fishingMovement,connectionState:connectionStateView,config:fishingConfig,logger});const fishingRecoveryPolicy=new FishingRecoveryPolicy({config:fishingConfig});
 ...
 const runtimeFailurePublisher=new RuntimeFailurePublisher({botId,eventBus,connectionAggregationMs:runtimeFailureConfig.connectionAggregationMs,logger});const modeCoordinator=new ModeCoordinator({botId,logger});const collectorB5Config=configuration.registry.require('collectorB5Mode');const collectorB5Mode=new CollectorB5ModeService({botId,context,eventBus,island,skyblock,skyblockReadiness:skyblockAutoJoin,skyTarget,movementManager,positionService,b1Materials,b5Planning,b5Automation,modeCoordinator,failurePublisher:runtimeFailurePublisher,failurePolicy,config:collectorB5Config,dailyRecovery:configuration.registry.require('dailyRecovery'),logger});const fishingMode=new FishingModeService({botId,eventBus,connectionState:connectionStateView,connectionControl:connectionManager,skyblockReadiness:skyblockAutoJoin,skyTarget,afkAreas,fishing,island,movement:fishingMovement,movementProbe:fishingMovementProbe,positionGuard:fishingPositionGuard,worldReadiness:fishingWorldReadiness,recoveryPolicy:fishingRecoveryPolicy,modeCoordinator,failurePublisher:runtimeFailurePublisher,failurePolicy,config:fishingConfig,logger});
 const capabilityRegistry=new CapabilityRegistry({botId});
 const capabilities={
  connection:connectionManager,commands:commandService,'slash-command':slashCommandService,'sky-commands':skyCommandService,gui:guiManager,'gui-identity':guiIdentityEngine,movement:movementManager,rotation:rotationService,position:positionService,inventory:inventoryReader,
 ...
 const collectorDefinition=shared.modeCatalog.require('collector-b5');const fishingDefinition=shared.modeCatalog.require('fishing');const collectorB5ModeAdapter=new LegacyModeAdapter({modeId:collectorDefinition.id,service:collectorB5Mode,modeContext,requiredCapabilities:collectorDefinition.requiredCapabilities});const fishingModeAdapter=new LegacyModeAdapter({modeId:fishingDefinition.id,service:fishingMode,modeContext,requiredCapabilities:fishingDefinition.requiredCapabilities});
 const servicesByName={collectorB5Mode:collectorB5ModeAdapter,fishingMode:fishingModeAdapter,b5CraftMode,...customModes};
