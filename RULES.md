...
- Quantity crafting hỗ trợ `1`, `64` và `ALL`. Với `b2InputSource=storage`, sau khi storage safety xác nhận `/kho` ổn định, B1→B2 được phép dùng `ALL` cho material planner chọn; `ALL` chỉ là quantity strategy, không phải quyền craft mù. Với `b2InputSource=inventory`, B1→B2 phải dùng lượng planner kiểm soát, giữ capacity cho B2 output và không dùng `ALL`. Nếu B1→B2 storage-direct `ALL` làm inventory đầy, phải cất đúng một stack B2 của material hiện tại vào `/pv 2`, verify có ít nhất một slot trống, rồi B2→B3 dùng `ALL`. Carry stack này phải được planner nhìn thấy/rút lại ở batch sau thay vì để tích lẻ vô hạn trong PV2. B4/B5 vẫn theo lượng planner cần; không suy rộng `ALL` sang B4/B5; B5 cuối không dùng `ALL`.
- Acquisition B1 inventory phải đi `/kho` overview → đúng material detail → semantic withdraw → numeric quantity, verify inventory delta và connection generation trước B2. Không hard-code slot; không mặc định dùng `1 stack`/`đầy inventory`; click mất ACK phải reconcile inventory trước khi quyết định retry để không rút đôi. Config cũ thiếu `b2InputSource` luôn fallback `storage` và tuyệt đối không mở withdrawal GUI.
