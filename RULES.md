...
- `/kho` là nguồn sự thật đầy đủ để đọc amount/capacity và tính coverage B1 trên cả raw/phôi/block. `/kho sell` chỉ mở Sell GUI của server; GUI bán có thể không hiển thị raw, nên tuyệt đối không dùng Sell GUI để suy ra full storage. Sell GUI chỉ được dùng để resolve/click/verify các form thực sự xuất hiện; left-click bán `1`, right-click bán `64`, Shift+left bán `ALL`, nhưng B1 production mặc định cấm `SELL ALL`. Ngoại lệ duy nhất là exact safe `sellId` nằm trong `storage.sell.fastDisposableSellAllIds`; quyền này không mở ALL cho resource khác. Lore Sell GUI còn có thể chứa số hướng dẫn click `1/64`, nên amount từ Sell GUI chỉ được coi là authoritative khi reader xác định được label storage thật; startup/pressure và fast-disposable ALL vẫn checkpoint bằng full `/kho`.
...
- Mode B5 chạy liên tục không có tổng target/batch và **không có cooldown B5**: hễ tier nào đủ điều kiện thì chế ngay, luôn ưu tiên đẩy `B5 > B4 > B3 > B2`. Mỗi lần explicit `enable`, trước production phải chạy đúng một startup storage-safety gate: đọc full `/kho`, tính coverage raw+phôi+block theo nhu cầu B1 thật cho một B5, giữ `startupReserveCoverage` là hard reserve cho resource thường nhưng chỉ trim tương đối tới `startupStopCoverage` bằng block-sale `64`; không đuổi một coverage tuyệt đối hard-code. Resource fast-disposable explicit không tham gia reserve/stop-band gate: đầu mỗi B5 cycle phải nén base→block rồi SELL ALL exact configured safe sellId tối đa một episode, full `/kho` checkpoint và chuyển sang production; input mới không được kéo dài episode. HIGH/CRITICAL pressure chỉ được mở episode mới sau cooldown. Trong nhóm tạo B2 mới, chain thường/chậm giữ thứ tự trước và fast-disposable bị đẩy về sau để tự tích trong lúc các chain khác chạy. Thiếu fast-disposable khi tới lượt craft là WAITING_MATERIALS bình thường. Gate thường bán theo các checkpoint full `/kho` có giới hạn và không được báo COMPLETE nếu sau checkpoint vẫn còn block surplus thường có thể bán an toàn. Material thường chưa đủ reserve hoặc surplus chỉ nằm ở raw/phôi thì bỏ qua. Gate này không chạy lại khi pause/resume hoặc reconnect; chỉ chạy lại sau disable rồi enable mới. Sau gate, B1→B2 có thể dùng guarded `ALL`; B2→B3 dùng `ALL`; B5 cuối không dùng `ALL`.
...
- Khi phải bán, coverage phải lấy từ full `/kho`: raw + phôi + block quy đổi về effective B1. **Sell candidate chỉ được là form `sellId` an toàn đã cấu hình:** với family nén được, `sellId` bắt buộc bằng `blockId`; riêng family không có block như cobblestone chỉ được bán base form khi `sellId === baseId` và ratio đúng `1:1`. Phôi/raw của mineral nén được vẫn bảo vệ reserve cấu hình nhưng bị bỏ qua ở executor; không bán phôi lẻ. Vì B1 được NPC cấp liên tục, production mặc định bán form an toàn theo `64`, không dùng click `1` để đuổi một coverage hard-code; dừng khi một click 64 tiếp theo sẽ làm family xuống dưới reserve. Runtime pressure bán theo burst có giới hạn, checkpoint full `/kho` giữa các burst; nếu inflow che mất delta hoặc làm pressure tăng, burst sau được tăng có trần thay vì restart mode/vòng vô hạn. `SELL ALL` vẫn cấm mặc định ngoài whitelist fast-disposable; resource whitelist bị loại khỏi reserve-sale selector để không vừa ALL xong lại quay vào vòng 64-sale. Fast-disposable ALL phải at-most-once theo B5 cycle, claim episode trước side effect để timeout không gây click ALL lần hai, compact trước, full `/kho` checkpoint sau và không bán intermediate B2–B5. Nếu surplus mineral thường hiện chỉ nằm ở raw/phôi, startup bỏ qua; các maintenance pass sau nung/nén về block rồi protection mới bán block.
